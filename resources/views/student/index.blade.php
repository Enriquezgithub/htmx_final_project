...
@section('content')
    ...
    {{-- <div class="fixed inset-0 flex items-center -mt-10 justify-center bg-gray-800 bg-opacity-50 @if ($errors->any()) @else hidden @endif"
        ...
        <div class="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:max-w-lg sm:w-full">
            <div class="px-4 py-5 sm:p-6">
                ...
                <form hx-post="/api/student" hx-target="#display-student" hx-swap="innerHTML" hx-trigger="submit"
                    method="POST" hx-on::after-request="this.reset()">
                    @csrf
                    <div class="mt-2">
                        <p class="text-sm text-gray-500">
    ...
    <div class="flex justify-between">
        <div class="text-xl font-bold" id="hidden">
            Student <span id="hiddenText">Record</span><span class="hidden" id="showText">Detail</span>
        </div>
        ...
        <div id="back" class="hidden">
            <a href="/student" class="bg-green-600 py-2 px-3 text-white rounded">Back</a>
        </div>

    </div>

    <div class="mt-3" id="display-student" hx-trigger="load" hx-get="/api/student" hx-swap="innerHTML">
    ...
    <script>
        ...
        function closeEditModal() {
            document.getElementById('EditTrigger').classList.add('hidden');
